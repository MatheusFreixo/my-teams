...
int change_user_status(steams_t *server, char *name, int client_fd, bool expected_status)
{
    users_t *tmp = NULL;
    LIST_FOREACH(tmp, &server->users, entry){
...
void add_user_to_list(steams_t *server, char *name, int client_fd)
{
    ...
    users_t *tmp = LIST_FIRST(&server->users);
}
...
void user_log_in(steams_t *server, char *name, int client_fd)
{
    ...
    char *id = strdup(get_user_id(server, name));
    strcpy(msg, "LOGIN ");
    strcat(msg, id);
...
void user_log_out(steams_t *server, int client_fd)
{
    ...
    name = get_user_name(server, client_fd);
